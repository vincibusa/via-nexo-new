    ...
    <ProtectedRoute requiredRole="admin">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold">Gestione Locali</h1>
            <p className="text-sm sm:text-base text-muted-foreground">Gestisci tutti i locali della piattaforma</p>
          </div>
          <Button asChild className="min-h-[44px] w-full sm:w-auto">
            <Link href="/admin/places/new">
              <Plus className="mr-2 h-4 w-4" />
              <span className="sm:inline">Nuovo Locale</span>
            </Link>
          </Button>
        </div>
              ...
              {loading ? (
                ...
                places.map((place) => (
                  <TableRow key={place.id}>
                    ...
                    <TableCell>
                      {place.owner?.display_name || place.owner?.email || '-'}
                    </TableCell>
